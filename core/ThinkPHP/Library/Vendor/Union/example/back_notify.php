...
try {
    ...
    //以下仅用于测试
    file_put_contents('./Api/Runtime/Logs/unionpay/'.date('y_m_d',time()).'.txt', var_export($arr_ret, true));

}
catch(Exception $exp) {
    //后台通知出错
    file_put_contents('./Api/Runtime/Logs/unionpay/'.date('y_m_d',time()).'.txt', var_export($exp, true));
}
